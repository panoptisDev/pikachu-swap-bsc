...
export const useWrappedBNB = (address: string) => {
  const { library } = useActiveWeb3React()
  return useMemo(() => getwrappedBNBContract(address, library.getSigner()), [address, library])
}
export const useSousChefV2 = (id) => {
  const { library } = useActiveWeb3React()
  return useMemo(() => getSouschefV2Contract(id, library.getSigner()), [id, library])
}
export const useLaunchpadv1Contract = (address: string) => {
  const { library } = useActiveWeb3React()
  return useMemo(() => getLaunchpadv1Contract(address, library.getSigner()), [address, library])
}
